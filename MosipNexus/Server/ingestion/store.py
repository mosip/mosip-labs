...
Reads crawled JSON files, chunks the content, embeds with the configured
HuggingFace model (``EMBED_MODEL``), and upserts into pgvector collections:

  - ``{PRODUCT_SLUG}_docs``       — documentation chunks
  - ``{PRODUCT_SLUG}_community``  — community Q&A chunks
  - ``{PRODUCT_SLUG}_github``     — GitHub issue chunks
  - ``{PRODUCT_SLUG}_code``       — source code chunks
  - ``{PRODUCT_SLUG}_confluence`` — Confluence page chunks
  - ``{PRODUCT_SLUG}_jira``       — Jira ticket chunks

Public API:
  * ``build_embeddings`` — shared embedder
  * ``prepare_*_documents`` — JSON → LangChain Documents
  * ``ingest`` — full rebuild of one collection
  * ``ingest_incremental`` / ``delete_chunks_by_url`` — delta updates
...
def ingest(documents: list[Document], collection_name: str, embeddings: HuggingFaceEmbeddings) -> None:
    ...
    Args:
        ...
        collection_name: Target ``langchain_pg_collection.name``.
        embeddings: Shared HuggingFace embedder.

    Notes:
        The first batch uses ``pre_delete_collection=True``, wiping any prior
        vectors for this collection name.
    """
...
def delete_chunks_by_url(collection_name: str, url: str) -> None:
    ...
    Args:
        ...
        url: Value of ``cmetadata->>'source'`` to match.
    """
    engine = get_engine()
...
def prepare_generic_documents(file_path, source_type_label: str) -> list[Document]:
    ...
    Args:
        file_path: Path to the crawled JSON file.
        source_type_label: Metadata ``source_type`` value (e.g. ``confluence``).

    Returns:
        LangChain documents, or ``[]`` if the file is missing.
    """
