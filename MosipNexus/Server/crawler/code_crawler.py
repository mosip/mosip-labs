...
Fetches product source code files from GitHub repositories and prepares them
for embedding into the ``{PRODUCT_SLUG}_code`` pgvector collection.
...
Repo discovery:
  Uses the same ``get_org_repos()`` function as ``github_crawler`` — auto-discovers
  relevant repos and uses each repo's own ``default_branch``.

Strategy (smart, not exhaustive):
  1. Priority files  — *ErrorConstants*, *ServiceImpl*, *Controller*, etc.
  2. Standard files  — other allowed extensions, up to per-repo limit.
  3. Skip patterns   — test files, generated code, build output.

Public crawl helpers: ``crawl_repo``, ``crawl_all``, ``crawl_incremental``.

Output: ``data/{PRODUCT_SLUG}_code.json``
"""
...
from config.settings import (
    ...
    CRAWL_DELAY_SECS, GITHUB_API_BASE, GITHUB_TOKEN,
)
from crawler.github_crawler import get_org_repos

_GITHUB_API = GITHUB_API_BASE.rstrip("/")
...
def get_repo_file_tree(repo: str, branch: str = "master") -> tuple[list[dict], str]:
    ...
    Falls back to ``main``, ``master``, ``develop`` if the requested branch
    does not exist. Only blob entries matching include/exclude filters are
    returned.

    Args:
        repo: Full name ``owner/name``.
        branch: Preferred branch (usually the repo default).

    Returns:
        ``(entries, resolved_branch)`` where each entry has ``path`` and ``sha``.
    """
...
def fetch_file_content(repo: str, file_path: str) -> str | None:
    ...
    Returns:
        UTF-8 text, or ``None`` if missing / too large / undecodable.
    """
...
def crawl_all(repos: list[dict] | None = None) -> list[dict]:
    """Crawl all product org repos for source code.

    Args:
        repos: Optional list of dicts with ``full_name`` and ``default_branch``
            (same shape as ``get_org_repos()``). When ``None``, auto-discovers.

    Returns:
        Combined list of file dicts for JSON / ingestion.
    """
...
def crawl_incremental(state: dict) -> list[dict]:
    ...
    Uses ``get_org_repos()`` to discover repos dynamically.

    Args:
        state: Crawl state dict (reads ``code.<repo>.file_shas``).

    Returns:
        Changed file dicts (may include ``_sha`` for state updates).
    """
