...
Reads the product docs sitemap (``DOCS_SITEMAP_URL``), follows child sitemaps
recursively, fetches every documentation page, converts HTML to clean Markdown
(with tables rewritten as prose for better embeddings), and writes results to
``data/{PRODUCT_SLUG}_docs.json``.

Public entry points:
  * ``get_all_page_urls`` / ``fetch_page_as_markdown`` — building blocks
  * ``crawl_fallback`` — DFS when sitemap is empty
  * ``crawl_incremental`` — used by ``run_update.py`` for delta crawls
  * ``__main__`` — full crawl to JSON
"""
...
def get_all_page_urls(sitemap_url: str, visited: set | None = None) -> list[str]:
    ...
    Args:
        sitemap_url: Root or child sitemap URL.
        visited: Internal set to avoid cycles (created if omitted).

    Returns:
        Flat list of HTML page URLs (``.xml`` locs excluded).
    """
    if visited is None:
        visited = set()
...
def fetch_page_as_markdown(url: str) -> str:
    ...
    Args:
        url: Absolute documentation page URL.

    Returns:
        Markdown string of the main content region.
...
def crawl_fallback(depth: int = 3) -> list[dict]:
    """Depth-first crawl from ``DOCS_BASE_URL`` when the sitemap is absent.

    Args:
        depth: Max link depth from the base URL.

    Returns:
        List of ``{url, content, source_type}`` page dicts.
    """
    visited, docs = set(), []
