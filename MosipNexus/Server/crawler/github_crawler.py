...
from config.settings import (
    CRAWL_DELAY_SECS, GITHUB_API_BASE, GITHUB_FILE, GITHUB_MAX_ISSUES,
...
def crawl_repo(repo: str, max_issues: int = GITHUB_MAX_ISSUES) -> list[dict]:
    ...
    Args:
        repo: Full name such as ``mosip/id-authentication``.
        max_issues: Cap on issues fetched for this repo.

    Returns:
        List of issue dicts with ``url``, ``title``, ``content``, ``repo``, etc.
    """
    print(f"\n  Crawling {repo} ...")
    _check_rate_limit()
...
def crawl_incremental(state: dict) -> list[dict]:
    ...
    Uses ``get_org_repos()`` to discover repos dynamically — new org repos are
    picked up automatically on the next incremental run.

    Args:
        state: Crawl state dict (reads ``github.<repo>.max_issue_number``).

    Returns:
        Newly discovered issue dicts ready for embedding.
    """
